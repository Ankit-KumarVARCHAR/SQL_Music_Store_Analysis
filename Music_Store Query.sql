...
SELECT artist.name AS "Artist Name", COUNT(track.track_id) AS "Track Count", genre.name AS "Genre Name"
FROM artist
...
SELECT 
    ...
    SUM(invoice_line.unit_price*invoice_line.quantity) AS TotalSpent
FROM 
    customer
JOIN 
...
GROUP BY 
    customer.customer_id, artist.artist_id
ORDER BY 
    TotalSpent DESC;
